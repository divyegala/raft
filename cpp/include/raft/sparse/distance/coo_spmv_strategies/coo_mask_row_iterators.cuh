...
class chunked_mask_row_it : public mask_row_it<value_idx> {
 public:
  ...
  void init() {
    ...
    fill_chunk_indices();
    // raft::print_device_vector("n_chunks_per_row", n_chunks_per_row.data().get(), this->n_rows + 1, std::cout);
    // raft::print_device_vector("chunk_indices", chunk_indices.data().get(), total_row_blocks, std::cout);
  }
